...
def PIE_boxes_display(pred_bbox, confidence_min = 0.7):
    path = 'C:/Users/Phili/Studium/Studium/SUPAERO/PIE/YOLO/tensorflow-yolov4-tflite-master/yolov4_result.npy'
    mes_real = np.load(path)
    valid_detec = pred_bbox[3][0]
    result=[]
    for i in range(0,valid_detec):
        if pred_bbox[1][0][i]>confidence_min:
            tracker_number = pred_bbox[2][0][i]
            X_mean = ((pred_bbox[0][0][i][1]+pred_bbox[0][0][i][3])/2.)
            Y_mean = ((pred_bbox[0][0][i][0]+pred_bbox[0][0][i][2])/2.)
            confidence = pred_bbox[1][0][i]
            result.append([tracker_number, X_mean, Y_mean, confidence])
    result = np.array(result)
    results = np.append(mes_real, [[-1, 0, 0, 0]], axis=0)
    try:
        results = np.append(results, result, axis=0)
    except:
        print('No markers found')
    np.save('yolov4_result.npy', results)
